...
import { MessageCircle, CalendarCheck, GraduationCap, Camera, Check, Award, Building, Sparkles } from "lucide-react";
...
import { brandProducts as staticBrandProducts } from "@/data/brand-products";
      ...
      <div className="p-8 sm:p-10 flex flex-col h-full relative z-10">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-2 mb-3">
            <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-[8px] font-black uppercase tracking-widest bg-gray-100 text-gray-500 border border-gray-200/50">
              {isAffiliate ? "🎓 Pelatihan" : "📸 Studio Foto"}
            </span>
            {pkg.category && (
              <span className="text-[9px] font-bold text-[#C88A58] uppercase tracking-wider">
                {pkg.category}
              </span>
            )}
          </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          >
            ...
            {isAffiliate ? (
              <GraduationCap size={16} className="transition-transform group-hover:scale-110" />
            ) : (
              <CalendarCheck size={16} className="transition-transform group-hover:rotate-12" />
            )}
            {isAffiliate ? "Daftar Pelatihan" : "Booking Studio"}
          </a>
...
export default function PackagesPage() {
  const [activeCategory, setActiveCategory] = useState<"all" | "foto" | "affiliate">("all");
  const [loading, setLoading] = useState(true);
  const [studioPackages, setStudioPackages] = useState<PkgProps[]>([]);
  const [affiliatePackages, setAffiliatePackages] = useState<PkgProps[]>([]);
    ...
    async function loadData() {
      try {
        ...
        if (res.success && res.data) {
          ...
          // Filter studio packages: categories "Paket Foto" or "Studio Foto" or fallback checking
          const studio = dbProducts
            .filter((p: any) => p.category.toLowerCase().includes("foto") || p.sku.startsWith("pkg-"))
            .map((p: any) => ({
              id: p.id,
              name: p.name,
              price: p.price,
          ...
          // Filter affiliate packages: everything else
          const aff = dbProducts
            .filter((p: any) => !p.category.toLowerCase().includes("foto") && !p.sku.startsWith("pkg-"))
            .map((p: any) => ({
              id: p.id,
              name: p.name,
          ...
          setAffiliatePackages(aff.length > 0 ? aff : (staticBrandProducts as any[]).map(p => ({ ...p, category: p.categoryName })));
        } else {
          // Fallback to static
          setStudioPackages((staticPackages as any[]).map(p => ({ ...p, category: "Paket Foto" })));
          setAffiliatePackages((staticBrandProducts as any[]).map(p => ({ ...p, category: p.categoryName })));
        }
      } catch (err) {
  ...
  return (
    <main className="min-h-screen bg-[#FAFAF8] font-[family-name:var(--font-lato)]">
      ...
      <section className="relative pt-32 pb-16 lg:pt-44 lg:pb-24 overflow-hidden border-b border-[#3B2211]/5 bg-[#F8F6F4]/50">
        ...
        <div className="max-w-4xl mx-auto px-6 text-center relative z-10 space-y-6">
          ...
          <h1
            ...
            style={{ fontFamily: "var(--font-heading)" }}
          >
            Investasi Untuk Masa Depan <br />
            <span className="text-[#C88A58] italic underline decoration-[#C88A58]/30 underline-offset-8">Dan Momen Terbaik</span>
          </h1>

          <p className="text-gray-500 text-sm sm:text-base font-bold max-w-2xl mx-auto leading-relaxed">
            Pilih di antara layanan Studio Foto minimalis modern kami, atau program Pelatihan Partnership Kemitraan bernilai tinggi kami.
          </p>

          {/* Tab Filter Category Switcher */}
          <div className="flex bg-[#F0EFE9] p-1.5 rounded-2xl w-fit mx-auto shadow-sm gap-1 border border-black/5 mt-8">
            <button
              onClick={() => setActiveCategory("all")}
              className={`flex items-center gap-2 px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                activeCategory === "all"
                  ? "bg-[#3B2211] text-white shadow-md shadow-[#3B2211]/25"
            ...
            <button
              ...
              className={`flex items-center gap-2 px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                activeCategory === "foto"
                  ? "bg-[#C88A58] text-white shadow-md shadow-[#C88A58]/25"
                  : "text-gray-400 hover:text-[#3B2211]"
              }`}
            >
              <Camera size={13} />
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 py-16 sm:py-24">
        {loading ? (
          ...
          <div className="space-y-20">
            {/* Studio Foto Section */}
            {(activeCategory === "all" || activeCategory === "foto") && (
              <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {studioPackages.map((pkg) => (
                    <PackageCard key={pkg.id} pkg={pkg} isAffiliate={false} />
                  ))}
                </div>
              </div>
            )}
            ...
            {(activeCategory === "all" || activeCategory === "affiliate") && (
              <div className="space-y-8 pt-8">
                <div className="flex items-center gap-3 border-b border-[#3B2211]/5 pb-4">
                  ...
                  <div>
                    <h2 className="text-xl font-black text-[#3B2211]" style={{ fontFamily: "var(--font-heading)" }}>
                      Program Pelatihan Partnership Kemitraan
                    </h2>
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-0.5">
                      Program Kelas & Pendampingan Bisnis Profesional
                    </p>
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {affiliatePackages.map((pkg) => (
                    <PackageCard key={pkg.id || pkg.sku} pkg={pkg} isAffiliate={true} />
                  ))}
                </div>
              </div>
            )}
      ...
      <div className="bg-[#F8F6F4]/30 border-t border-[#3B2211]/5">
        <div className="max-w-4xl mx-auto px-6 sm:px-8 py-20 lg:py-24 space-y-12">
          <div className="text-center">
            <p className="text-[9px] font-black tracking-[0.3em] text-[#C88A58] uppercase mb-3">
              FAQ
            </p>
            <h2
              className="text-3xl font-black text-[#3B2211]"
              style={{ fontFamily: "var(--font-heading)" }}
            >
              Pertanyaan <span className="text-[#C88A58]">Umum</span>
            </h2>
            <p className="text-gray-500 font-bold text-xs mt-3">
              Butuh panduan lebih lanjut? WhatsApp kami selalu aktif.
            </p>
          </div>

          <div className="bg-white rounded-3xl border border-[#E0E0DA] p-6 sm:p-8 space-y-6 shadow-sm">
            <div className="space-y-4">
              <h3 className="font-black text-sm text-[#3B2211]">Bagaimana cara memesan paket foto studio?</h3>
              <p className="text-xs text-gray-500 leading-relaxed font-bold">
                Pilih tab &quot;Studio Foto&quot; di atas, klik &quot;Booking Studio&quot; pada paket yang diinginkan, kemudian isi formulir penjadwalan tanggal & jam sesi foto Anda serta selesaikan pembayaran.
              </p>
            </div>
            <div className="h-px bg-gray-100" />
            <div className="space-y-4">
              <h3 className="font-black text-sm text-[#3B2211]">Bagaimana cara mendaftar program pelatihan kemitraan (affiliate)?</h3>
              <p className="text-xs text-gray-500 leading-relaxed font-bold">
                Pilih tab &quot;Program Pelatihan&quot;, tentukan kelas yang ingin Anda ikuti, klik &quot;Daftar Pelatihan&quot;, isi data diri Anda pada formulir pendaftaran khusus, dan tim kami akan segera memproses pendaftaran Anda.
              </p>
            </div>
          </div>
        </div>
